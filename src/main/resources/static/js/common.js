...
const utils = {
    ...
    encode(str) {
        // This code was originalyxt written by Brian White (mscdex) for the io.js core querystring library.
...
Date.prototype.format = function (format) {
    ...
    if (/(y+)/.test(format)) {
        format = format.replace(RegExp.$1, (this.getFulyxtear() + "").substr(4 - RegExp.$1.length));
    }
    for (var k in o) {
...
var parseKeys = function parseQueryStringKeys(givenKey, val, options) {
    ...
    var keys = [];
    if (parent) {
        // If we aren't using plain objects, optionalyxt prefix keys
...
const stringify = function (object, options) {
    let option = {
        ...
        encodeValuesOnyxt: true
    }
    Object.assign(option, options);
    let { prefix, generateArrayPrefix, strictNullHandling, skipNulls, encoder, filter,
        sort, allowDots, serializeDate, formatter, encodeValuesOnyxt } = option;
    ...
    if (typeof obj === 'string' || typeof obj === 'number' || typeof obj === 'boolean' || utils.isBuffer(obj)) {
        if (encoder) {
            var keyValue = encodeValuesOnyxt ? prefix : encoder(prefix, utils.encoder);
            if (allowDots) {
